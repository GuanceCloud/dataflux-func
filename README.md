...
如系统部署后通过了最初的单机验证阶段，需要将数据库切换至外部数据库（如：阿里云RDS），可根据以下步骤进行操作：

*注意：当使用外部数据库时，应确保MySQL版本为5.7，Redis版本为4.0以上*

*注意：DataFlux Func 不支持集群部署的Redis，需要高可用时可选择主从部署的Redis*
...
*注意：DataFlux Func 不支持集群部署的Redis，使用集群部署的Redis 可能发生各种奇怪的问题*
