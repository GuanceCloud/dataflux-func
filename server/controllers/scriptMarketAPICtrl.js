...
function _getDefaultScriptMarketReadmeContent(scriptMarket, scriptSets) {
  // 脚本市场信息
  var content = [
    ...
    `## 1. 描述 / Description`,
    `${toolkit.toMarkdownTextBlock(scriptMarket.description) || '*没有具体描述 / No description*'}`,
  ];

  // 脚本集信息
  if (!toolkit.isNothing(scriptSets)) {
    content.push(`## 2. 脚本集 / Script Sets`,
                  `本脚本市场包含以下脚本集：  `,
                  `This Script Market contains the following Script Sets:   `);

    var _table = [
      [ '#', 'ID', '标题 / title', '位置 / Location' ],
    ];
    scriptSets.forEach(function(s, index) {
      _table.push([
        `${index + 1}`,
        `\`${s.id}\``,
        `${s.title || '-'}`,
        `[${CONFIG.SCRIPT_MARKET_SCRIPT_SET_DIR}/${s.id}](${CONFIG.SCRIPT_MARKET_SCRIPT_SET_DIR}/${s.id})`,
      ])
    });
...
function _getDefaultScriptSetReadmeContent(scriptSet) {
  ...
  var content = [
    ...
    `## 1. 描述 / Description`,
    `${toolkit.toMarkdownTextBlock(scriptSet.description) || '*没有具体描述 / No description*'}`,
  ...
  if (!toolkit.isNothing(scriptSet.scripts)) {
    // 脚本信息
    content.push(`## 3. 脚本 / Scripts`,
                  `本脚本集包含以下脚本：  `,
                  `This Script Set contains the following Scripts:   `);

    var _table = [
      [ '#', 'ID', '标题 / title', '行数', '字符数', 'MD5', '更新时间 / Update Time' ],
    ];
    var codeLines = 0;
    var codeChars = 0;
...
function _syncToGitRepo(git, note, callback) {
  async.series([
    // git add
    function(asyncCallback) {
...
var SCRIPT_MARKET_PUSH_FUNC_MAP = {
  git: function(locals, scriptMarket, pushContent, note, callback) {
    ...
    async.series([
      ...
      function(asyncCallback) {
        try {
          ...
          if (_isAutoGeneratedFileOrNothing(scriptMarketReadmeFilePath)) {
            var scriptMarketReadmeData = _getDefaultScriptMarketReadmeContent(pushContent.scriptMarket, pushContent.scriptSets);
      ...
      // 同步至 Git
      function(asyncCallback) {
        _syncToGitRepo(git, note, asyncCallback);
      },
    ], callback);
  },
    ...
    async.series([
      ...
      function(asyncCallback) {
        pullScriptSetIds.forEach(function(scriptSetId) {
          ...
          var metaFilePath = path.join(scriptSetDir, CONFIG.SCRIPT_MARKET_META_FILE);
          var metaData     = toolkit.safeReadFileSync(metaFilePath, 'yaml');
          if (!metaData || !Array.isArray(metaData.scripts)) return;

          // 读取代码
          metaData.scripts.forEach(function(script) {
            var codeFilePath = path.join(scriptSetDir, CONFIG.SCRIPT_MARKET_META_FILE);
            script.code = toolkit.safeReadFileSync(codeFilePath);
          });

          pullContent.scriptSets.push(metaData);
        })
...
exports.push = function(req, res, next) {
  ...
  async.series([
    ...
    // 发布脚本集
    function(asyncCallback) {
      SCRIPT_MARKET_PUSH_FUNC_MAP[scriptMarket.type](req.locals, scriptMarket, pushContent, note, asyncCallback);
    },
  ], function(err) {
    if (err) return next(err);
