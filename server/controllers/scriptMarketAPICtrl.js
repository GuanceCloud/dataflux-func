  ...
  aliyun_oss: function(locals, scriptMarket, callback) {
    ...
    async.series([
      ...
      function(asyncCallback) {
        ...
        var cmd = path.join(__dirname, '../../tools/ossutil64');
        var cmdArgs = [
          'cp', ossPath, localPath, '-r', '-f',
          '-e', ossEndpoint,
        ...
        childProcess.spawn(cmd, cmdArgs, cmdOpt).on('close', function(code) {
          var err = code === 0 ? null : new E('EBizCondition', 'Script Market operation Failed');
...
var SCRIPT_MARKET_SYNC_REPO_FUNC_MAP = {
  ...
  aliyun_oss: function(locals, scriptMarket, author, note, callback) {
    var lockKey     = toolkit.getCacheKey('lock', 'scriptMarketOperation');
    var lockValue   = Date.now().toString();
    var lockAge     = CONFIG._SCRIPT_MARKET_OPERATION_LOCK_AGE;
    var lockWaitAge = CONFIG._SCRIPT_MARKET_OPERATION_LOCK_WAIT_AGE;
  ...
  aliyun_oss: function(locals, scriptMarket, callback) {
    ...
    async.series([
      ...
      function(asyncCallback) {
        ...
        childProcess.exec(cmd, cmdArgs, cmdOpt, function(err, stdout, stderr) {
          console.log(err)
...
function _getLocalAbsPath(scriptMarket) {
  switch(scriptMarket.type) {
    ...
    case 'aliyun_oss':
      var endpoint = new URL(scriptMarket.configJSON.endpoint);
      var localAbsPath = path.join(
          CONFIG.RESOURCE_ROOT_PATH,
          CONFIG.SCRIPT_MARKET_ALIYUN_OSS_REPO_DIR,
          endpoint.hostname,
          scriptMarket.configJSON.bucket,
          scriptMarket.configJSON.folder);
      return localAbsPath;
  }
};
...
// 脚本市场 - 列出脚本集
function _listScriptSets(scriptMarket, callback) {
  var scriptSets = null;
...
exports.listScriptSets = function(req, res, next) {
  ...
  async.series([
    ...
    // 列出脚本市场脚本集
    function(asyncCallback) {
      _listScriptSets(scriptMarket, function(err, _scriptSets) {
        if (err) return asyncCallback(err);
