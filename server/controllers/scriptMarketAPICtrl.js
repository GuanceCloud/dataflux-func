...
function _getDefaultScriptMarketReadmeContent(scriptMarket, pushContent) {
  ...
  if (toolkit.notNothing(pushContent.deleteScriptSets)) {
    ...
    pushContent.deleteScriptSets.forEach(function(scriptSet, index) {
      _table.push([
        ...
        `\`${scriptSet.id}\``,
        `${scriptSet.title || '-'}`,
        `${CONFIG.SCRIPT_EXPORT_SCRIPT_SET_DIR}/${scriptSet.id}`,
        deleteTimeCN,
      ])
    });
  ...
  // 脚本集总表
  var scriptSets = _listAllMetaData(scriptMarket);
...
function _downloadAliyunOSSFolder(scriptMarket, localPath, callback) {
  ...
  var localPathBackup = `${localPath}.backup`;

  // 清理目录
  fs.emptyDirSync(localPath);

  var cmdArgs = [
    'sync', ossPath, (localPath + '/'), '-f', '--delete',
    '--backup-dir', (localPathBackup + '/'),
    '-e', ossEndpoint,
  ...
  childProcess.execFile(OSSUTIL_CMD, cmdArgs, function(err, stdout, stderr) {
    if (err) return callback(stderr.toString() || stdout.toString());

    fs.removeSync(localPathBackup);

    return callback();
  });
};

function _downloadHTTPServerFolder(scriptMarket, localPath, callback) {
  // 清理目录
  fs.emptyDirSync(localPath);
  var baseURL = `${scriptMarket.configJSON.url}/`;

  // 下载 FILE_INDEX
  var requestOptions = {
    forever: true,
    method : 'get',
    url    : baseURL + CONFIG.SCRIPT_MARKET_FILE_INDEX_FILE,
  };
...
var SCRIPT_MARKET_INIT_FUNC_MAP = {
  ...
  aliyunOSS: function(locals, scriptMarket, callback) {
    ...
    async.series([
      // 下载文件夹
      function(asyncCallback) {
        _downloadAliyunOSSFolder(scriptMarket, localPathTmp, asyncCallback);
      },
    ], function(err) {
      // 下载失败
      if (err) {
  ...
  httpServer: function(locals, scriptMarket, callback) {
    ...
    async.series([
      // 下载文件夹
      function(asyncCallback) {
        _downloadHTTPServerFolder(scriptMarket, localPathTmp, asyncCallback);
      },
    ], function(err) {
      // 下载失败
      if (err) {
...
// 脚本市场 - 准备
var SCRIPT_MARKET_PREAPRE_REPO_FUNC_MAP = {
  git: function(locals, scriptMarket, callback) {
    var localPath = _getLocalAbsPath(scriptMarket);
  ...
  aliyunOSS: function(locals, scriptMarket, callback) {
    ...
    async.series([
      ...
      // 下载整个目录
      function(asyncCallback) {
        _downloadAliyunOSSFolder(scriptMarket, localPath, asyncCallback);
      },
    ], function(err) {
      // 解锁
      locals.cacheDB.unlock(lockKey, lockValue);
  ...
  httpServer: function(locals, scriptMarket, callback) {
    ...
    async.series([
      ...
      // 下载整个目录
      function(asyncCallback) {
        _downloadHTTPServerFolder(scriptMarket, localPath, asyncCallback);
      },
    ], function(err) {
      // 解锁
      locals.cacheDB.unlock(lockKey, lockValue);
...
var SCRIPT_MARKET_SYNC_REPO_FUNC_MAP = {
  ...
  aliyunOSS: function(locals, scriptMarket, author, note, callback) {
    ...
    async.series([
      ...
      function(asyncCallback) {
        // 生成文件索引
        childProcess.execSync(`find . -type f > ${CONFIG.SCRIPT_MARKET_FILE_INDEX_FILE}`, { cwd: localPath, stdio: 'ignore' });

        var cmdArgs = [
          'sync', (localPath + '/'), ossPath, '-f', '--delete',
          '-e', ossEndpoint,
        ]
        if (scriptMarket.configJSON.accessKeyId && scriptMarket.configJSON.accessKeySecret) {
          cmdArgs.push(
            '-i', scriptMarket.configJSON.accessKeyId,
            '-k', scriptMarket.configJSON.accessKeySecret
          );
        ...
        childProcess.execFile(OSSUTIL_CMD, cmdArgs, function(err, stdout, stderr) {
          if (err) return asyncCallback(stderr.toString() || stdout.toString());
          return asyncCallback();
        });
      },
    ], function(err) {
...
// 脚本市场 - 列出所有 META
function _listAllMetaData(scriptMarket) {
  var localPath   = _getLocalAbsPath(scriptMarket);
  var allMetaData = [];
...
function _listScriptSets(locals, scriptMarket, callback) {
  ...
  async.series([
    function(asyncCallback) {
      SCRIPT_MARKET_PREAPRE_REPO_FUNC_MAP[scriptMarket.type](locals, scriptMarket, asyncCallback);
    },
    function(asyncCallback) {
      try {
        scriptSets = _listAllMetaData(scriptMarket);
        return asyncCallback();
      } catch(err) {
        return asyncCallback(err);
      }
...
exports.list = function(req, res, next) {
  ...
  async.series([
    ...
    scriptMarkets.forEach(function(scriptMarket) {
      // 获取 META 信息
      scriptMarket.scriptSets = _listAllMetaData(scriptMarket) || [];
...
exports.install = function(req, res, next) {
  ...
  async.series([
    ...
    // 获取拉取数据数据
    function(asyncCallback) {
      _pullFromScriptMarket(res.locals, scriptMarket, scriptSetIds, function(err, _pullContent) {
        if (err) return asyncCallback(err);
