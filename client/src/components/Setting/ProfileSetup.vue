...
'You are signed in as a integrated user via "{funcTitle}", please change your profile in the origin system': '当前用户为通过「{funcTitle}」登录的集成用户，请前往原系统进行操作'
...
User Profile: 用户信息
User Profile saved: 用户信息已保存
You are signed in as a integrated user via "{funcTitle}", please change your profile in the origin system: 當前用户為通過「{funcTitle}」登錄的集成用户，請前往原系統進行操作
</i18n>
...
You are signed in as a integrated user via "{funcTitle}", please change your profile in the origin system: 當前使用者為透過「{funcTitle}」登入的整合使用者，請前往原系統進行操作
</i18n>
          ...
          <el-col :span="15">
            <div class="setup-form">
              <el-form ref="form" label-width="135px" :model="form" :rules="formRules" :disabled="$store.getters.integratedSignInFuncId">
                <el-form-item>
                  <InfoBlock v-if="$store.getters.integratedSignInFuncId" type="error" :title="$t('You are signed in as a integrated user via &quot;{funcTitle}&quot;, please change your profile in the origin system', { funcTitle: $store.getters.integratedSignInFuncTitle })" />
                </el-form-item>
