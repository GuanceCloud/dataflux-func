...
Are you sure you want to disable the Batch?: 是否確認禁用此批處理？
Auth: 認證
Batch deleted: 批處理已刪除
Batch disabled: 批處理已禁用
Batch enabled: 批處理已啓用
Config: 配置
No Batch has ever been added: 從未添加過任何批處理
Show all contents: 展示全部內容
Task Record: 任務記錄
Using Batches, you can execute long and time-consuming Python functions: 使用批處理，可以執行長耗時的 Python 函數
lastFailed: '{t}執行失敗'
lastStarted: '{t}執行'
...
Batch deleted: 批處理已刪除
Batch disabled: 批處理已禁用
Batch enabled: 批處理已啟用
Config: 配置
No Batch has ever been added: 從未新增過任何批處理
Show all contents: 展示全部內容
Task Record: 任務記錄
Using Batches, you can execute long and time-consuming Python functions: 使用批處理，可以執行長耗時的 Python 函式
lastFailed: '{t}執行失敗'
lastStarted: '{t}執行'
lastSucceeded: '{t}執行成功'
</i18n>
          ...
          <el-table-column align="right" width="350">
            <template slot-scope="scope">
              <template v-if="funcTaskRecordCountMap[scope.row.id]">
                <el-link @click="common.goToTaskRecord({ origin: 'batch', originId: scope.row.id }, { hlDataId: scope.row.id })" :disabled="!funcTaskRecordCountMap[scope.row.id].count">
