...
Add members to allow other users to use the platform: 添加成員，允許其他用户使用本平台
Are you sure you want to disable the User?: 是否確認禁用此用户？
No User has ever been added: 從未添加過任何用户
Session: 會話
User disabled: 用户已禁用
User enabled: 用户已啓用
lastAccess: '{t}訪問'
...
Add members to allow other users to use the platform: 新增成員，允許其他使用者使用本平臺
Are you sure you want to disable the User?: 是否確認禁用此使用者？
No User has ever been added: 從未新增過任何使用者
Session: 會話
User disabled: 使用者已禁用
User enabled: 使用者已啟用
lastAccess: '{t}訪問'
sessionCount: '{n} 個會話'
</i18n>
          ...
          <el-table-column :label="$t('Session')" width="350">
            <template slot-scope="scope">
              <template v-if="scope.row.sessions.length > 0">
                <span v-if="scope.row.sessions[0].idle < 15 * 60" class="text-good"><i class="fa fa-fw fa-circle"></i> {{ $t('Online') }}</span>
                ...
                <el-tooltip v-if="scope.row.sessions.length > 1" effect="dark" placement="right">
                  <div slot="content">
                    <div v-for="s, i in scope.row.sessions">
                      ...
                      {{ $t('lastAccess', { t: T.toNow((T.getTimestamp() - s.idle) * 1000) }) }}
                    </div>
                  </div>
                ...
                {{ $t('lastAccess', { t: T.toNow((T.getTimestamp() - scope.row.sessions[0].idle) * 1000) }) }}
              </template>
              <template v-else>
                <span class="text-info">
