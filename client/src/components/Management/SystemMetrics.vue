...
export default {
  ...
  methods: {
    ...
    async updateChart() {
      ...
      let apiRes = await this.T.callAPI_get('/api/v1/monitor/system-metrics/do/get');
      if (!apiRes || !apiRes.ok) {
        this.isRefreshing = false;
        return;
      }

      let systemMetircs = apiRes.data;

      for (let metric in systemMetircs) {
        let chart = this.charts[metric];
        if (!chart) continue;

        let tsDataMap = systemMetircs[metric];
