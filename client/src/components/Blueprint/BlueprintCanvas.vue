...
Add Switch: 添加分支
Algorithm: 算法
Are you sure you want to deploy the Blueprint?: 是否確認部署此藍圖？
Assign by name: 按名稱指定
Basic Node: 基礎節點
Blueprint deployed: 藍圖已部署
Body: 請求體
Body Type: 請求體類型
Built-in Node: 內置節點
Calling Preview: 調用預覽
Cipher: 加密
Comming soon...: 敬請期待...
Content: 內容
Decipher: 解密
...
ELSE: 除此之外
Encode: 編碼
Enter this branch if no expression returns True: 沒有任何表達式返回 True 時，進入本分支
Featured Node: 功能節點
Fit View: 適合大小
IF: 如果
Input Field: 輸入字段
...
Min Value: 最小值
No parameter: 不傳遞參數
Open Setting Panel: 打開設置面板
Operation: 操作
Or double-click the node: 或鼠標雙擊節點
Output Field: 輸出字段
Parameter: 參數
Passed as first parameter: 作為第一個參數傳遞
Plain Text: 純文本
Please input a Python Expression which returns a bool for Switch: 請輸入一個返回布爾值的 Python 表達式
Python expression: Python 表達式
Redo: 重做
Reset: 重置
Secret: 密鑰
Selected: 已選中
...
URL: URL 地址
Undo: 撤銷
Unpack as named parameters: 以具名參數解包
Zoom In: 放大
Zoom Out: 縮小
</i18n>
...
ELSE: 除此之外
Encode: 編碼
Enter this branch if no expression returns True: 沒有任何表示式返回 True 時，進入本分支
Featured Node: 功能節點
Fit View: 適合大小
IF: 如果
Input Field: 輸入欄位
...
Min Value: 最小值
No parameter: 不傳遞引數
Open Setting Panel: 開啟設定面板
Operation: 操作
Or double-click the node: 或滑鼠雙擊節點
Output Field: 輸出欄位
Parameter: 引數
Passed as first parameter: 作為第一個引數傳遞
Plain Text: 純文字
Please input a Python Expression which returns a bool for Switch: 請輸入一個返回布林值的 Python 表示式
Python expression: Python 表示式
Redo: 重做
Reset: 重置
Secret: 金鑰
Selected: 已選中
...
URL: URL 地址
Undo: 撤銷
Unpack as named parameters: 以具名引數解包
Zoom In: 放大
Zoom Out: 縮小
</i18n>
