function genBaseCode(nodeList, linkList) {
  let startNode = nodeList.find(node => node.meta.type === 'startPoint');
  if (!startNode) return '';
  ...
  codeLines.push(``)
  codeLines.push(`def blueprint_end(prev_res):`);
  codeLines.push(`    return prev_res`)
  ...
  nodeList.forEach(node => {
    if (node.meta.code) {
      ...
      if (node.meta.type === 'branchPoint') {
        // 分支点有两个去处
        let trueLink  = outLinks.find(link => link.meta.type === 'branchTrueLink');
        let falseLink = outLinks.find(link => link.meta.type === 'branchFalseLink');
        codeLines.push(`    branch_res = entry_func(prev_res)`);
        codeLines.push(`    return '${trueLink.endId}' if branch_res else '${falseLink.endId}', prev_res`);
